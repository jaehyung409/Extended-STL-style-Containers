...
namespace j {
template <class Traits, bool Numeric = NumericKeyTraits<Traits>> class skip_list;
}

namespace j {
template <class Traits> class skip_list<Traits, false> {
  private:
    class _iterator;
...
template <class Traits> struct skip_list<Traits, false>::_skip_list_node {
    friend skip_list;
    friend _iterator;
...
template <class Traits> class skip_list<Traits, false>::_iterator {
    friend skip_list;
    friend _const_iterator;
...
template <class Traits> struct skip_list<Traits, false>::node_type { // Node handler implementation is incorrect.
...
template <class Traits> class skip_list<Traits, false>::node_forward_guard {
  private:
    node_ptr _ptr;
    node_allocator_type _node_alloc;
    size_type _level;
...
template <class Traits> class skip_list<Traits, false>::copy_guard {
  private:
...
namespace j {
template <class Traits> skip_list<Traits, false>::size_type skip_list<Traits, false>::_random_level() const {
    size_type level = 0;
...
template <class Traits> auto skip_list<Traits, false>::_construct_node(size_type level) -> node_forward_guard {
...
template <class Traits> void skip_list<Traits, false>::_deallocate_dummy_node() noexcept {
    using node_ptr_allocator = typename std::allocator_traits<node_allocator_type>::template rebind_alloc<node_ptr>;
    node_ptr_allocator _node_ptr_alloc(_node_alloc);
...
template <class Traits> void skip_list<Traits, false>::_deallocate_node(node_ptr node) noexcept {
...
auto skip_list<Traits, false>::_find_predecessors(K &&key) const -> array<node_ptr, MAX_LEVEL + 1> {
    array<node_ptr, MAX_LEVEL + 1> predecessors;
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
auto skip_list<Traits, false>::_find_predecessors_lower(K &&key) const -> array<node_ptr, MAX_LEVEL + 1> {
    array<node_ptr, MAX_LEVEL + 1> predecessors;
    node_ptr current = _dummy;
    for (size_type i = _max_level + 1; i > 0; --i) { // avoid unsigned int underflow
...
template <class Traits>
auto skip_list<Traits, false>::_find_predecessors(const_iterator position) const -> array<node_ptr, MAX_LEVEL + 1> {
...
template <class Traits>
void skip_list<Traits, false>::_update_predecessors(key_type key, array<node_ptr, MAX_LEVEL + 1> &predecessors) {
...
template <class Traits>
void skip_list<Traits, false>::_advance_predecessors(array<node_ptr, MAX_LEVEL + 1> &predecessors) {
    node_ptr current_node = predecessors[0]->_forward[0];
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
bool skip_list<Traits, false>::_is_duplicate(K &&key, node_ptr next) const {
...
auto skip_list<Traits, false>::_init_node(V &&value, size_type level) -> node_forward_guard {
...
template <class Traits> void skip_list<Traits, false>::_init_dummy() {
...
template <class Traits>
void skip_list<Traits, false>::_move_state(skip_list &&x) { // pre-require: _dummy is initialized (call _init_dummy())
    using std::swap;
...
template <class Traits> template <class Strategy> void skip_list<Traits, false>::_clone_tree(const skip_list &other) {
    if (other.empty()) {
        _init_dummy();
        _max_level = 0;
        _size = 0;
...
template <class Traits> void skip_list<Traits, false>::_destroy_tree() noexcept {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::const_iterator skip_list<Traits, false>::_find_lower_bound(K &&key) const {
    node_ptr current = _dummy;
    for (size_type i = _max_level + 1; i > 0; --i) {
        while (current->_forward[i - 1] != _dummy && _key_comp(current->_forward[i - 1]->_key(), key)) {
            current = current->_forward[i - 1];
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::const_iterator skip_list<Traits, false>::_find_upper_bound(K &&key) const {
    node_ptr current = _dummy;
    for (size_type i = _max_level + 1; i > 0; --i) {
        while (current->_forward[i - 1] != _dummy && !_key_comp(key, current->_forward[i - 1]->_key())) {
            current = current->_forward[i - 1];
...
template <class Traits>
skip_list<Traits, false>::node_ptr
skip_list<Traits, false>::_extract_node(const_iterator position,
...
template <class Traits>
void skip_list<Traits, false>::_insert_node(node_ptr new_node, array<node_ptr, MAX_LEVEL + 1> &predecessors) noexcept {
...
std::pair<typename skip_list<Traits, false>::iterator, bool> skip_list<Traits, false>::_emplace(size_type level,
                                                                                                Args &&...args) {
...
template <class Traits>
skip_list<Traits, false>::skip_list(const key_compare &comp, const allocator_type &alloc)
...
template <class Traits>
skip_list<Traits, false>::skip_list(const skip_list &other)
...
template <class Traits>
skip_list<Traits, false>::skip_list(const skip_list &other, const std::type_identity_t<allocator_type> &alloc)
    : _node_alloc(alloc), _key_comp(other._key_comp) {
    _clone_tree<_strategy_copy>(other);
}

template <class Traits>
skip_list<Traits, false>::skip_list(skip_list &&x)
...
template <class Traits>
skip_list<Traits, false>::skip_list(skip_list &&x, const std::type_identity_t<allocator_type> &alloc)
    : rng(std::move(x.rng)), coin_flip(std::move(x.coin_flip)), _max_level(0), _node_alloc(std::move(x._node_alloc)),
      _size(0), _key_comp(std::move(x._key_comp)) {
...
template <class Traits> skip_list<Traits, false> &skip_list<Traits, false>::operator=(const skip_list &x) {
...
template <class Traits>
skip_list<Traits, false> &skip_list<Traits, false>::operator=(skip_list &&x) noexcept(
...
template <class Traits> skip_list<Traits, false>::~skip_list() noexcept {
    _destroy_tree();
}

template <class Traits> skip_list<Traits, false>::iterator skip_list<Traits, false>::begin() noexcept {
    return iterator(_dummy->_forward[0]);
}

template <class Traits> skip_list<Traits, false>::const_iterator skip_list<Traits, false>::cbegin() const noexcept {
    return const_iterator(_dummy->_forward[0]);
}

template <class Traits> skip_list<Traits, false>::iterator skip_list<Traits, false>::end() noexcept {
    return iterator(_dummy);
}

template <class Traits> skip_list<Traits, false>::const_iterator skip_list<Traits, false>::cend() const noexcept {
    return const_iterator(_dummy);
}

template <class Traits> bool skip_list<Traits, false>::empty() const noexcept {
    return _size == 0;
}

template <class Traits> skip_list<Traits, false>::size_type skip_list<Traits, false>::size() const noexcept {
    return _size;
}

template <class Traits> skip_list<Traits, false>::size_type skip_list<Traits, false>::max_size() const noexcept {
    return std::allocator_traits<allocator_type>::max_size(get_allocator());
}

template <class Traits>
skip_list<Traits, false>::allocator_type skip_list<Traits, false>::get_allocator() const noexcept {
...
template <class... Args>
    requires std::constructible_from<typename Traits::value_type, Args &&...>
std::pair<typename skip_list<Traits, false>::iterator, bool> skip_list<Traits, false>::emplace(Args &&...args) {
    size_type new_node_level = _random_level();
...
template <class... Args>
    requires std::constructible_from<typename Traits::value_type, Args &&...>
skip_list<Traits, false>::iterator skip_list<Traits, false>::emplace_hint(const_iterator position, Args &&...args) {
    value_type val(std::forward<Args>(args)...);
    key_type key;
    if constexpr (_IS_SET) {
        key = val;
...
void skip_list<Traits, false>::insert(InputIter first, InputIter last) {
    if (first == last)
        return;
...
template <class Traits> skip_list<Traits, false>::node_type skip_list<Traits, false>::extract(const_iterator position) {
    return node_type{_extract_node(position, _find_predecessors(position)), get_allocator()};
}

template <class Traits> skip_list<Traits, false>::insert_return_type skip_list<Traits, false>::insert(node_type &&nh) {
...
template <class Traits>
skip_list<Traits, false>::iterator skip_list<Traits, false>::insert(const_iterator position, node_type &&nh) {
...
template <class Traits> skip_list<Traits, false>::iterator skip_list<Traits, false>::erase(const_iterator position) {
...
template <class Traits>
skip_list<Traits, false>::iterator skip_list<Traits, false>::erase(const_iterator first, const_iterator last) {
...
template <class Traits>
void skip_list<Traits, false>::swap(skip_list &x) noexcept(
...
template <class Traits> void skip_list<Traits, false>::clear() noexcept {
    node_ptr current = _dummy->_forward[0];
    while (current != _dummy) {
        node_ptr next = current->_forward[0];
        _deallocate_node(current);
...
// template <class SourceTree>
// requires IsMergeable<Traits, SourceTree>
// void skip_list<Traits, false>::merge(SourceTree &source) {}

template <class Traits> void skip_list<Traits, false>::merge(skip_list &source) {
...
template <class Traits> skip_list<Traits, false>::key_compare skip_list<Traits, false>::key_comp() const {
    return _key_comp;
}

template <class Traits> skip_list<Traits, false>::value_compare skip_list<Traits, false>::value_comp() const {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::iterator skip_list<Traits, false>::find(K &&key) {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::const_iterator skip_list<Traits, false>::find(K &&key) const {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::size_type skip_list<Traits, false>::count(K &&key) const {
    if constexpr (_MULTI) {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
bool skip_list<Traits, false>::contains(K &&key) const {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::iterator skip_list<Traits, false>::lower_bound(K &&key) {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::const_iterator skip_list<Traits, false>::lower_bound(K &&key) const {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::iterator skip_list<Traits, false>::upper_bound(K &&key) {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, false>::const_iterator skip_list<Traits, false>::upper_bound(K &&key) const {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
std::pair<typename skip_list<Traits, false>::iterator, typename skip_list<Traits, false>::iterator>
skip_list<Traits, false>::equal_range(K &&key) {
    auto lower = _find_lower_bound(std::forward<K>(key));
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
std::pair<typename skip_list<Traits, false>::const_iterator, typename skip_list<Traits, false>::const_iterator>
skip_list<Traits, false>::equal_range(K &&key) const {
...
namespace j {
namespace detail {
template <typename Key, typename Compare> struct _key_limits_helper {
    static Key min() {
        if constexpr (std::is_same_v<Compare, std::less<Key>> || std::is_same_v<Compare, std::less<>>) {
...
template <class Traits> class skip_list<Traits, true> {
  private:
    class _iterator;
    class _const_iterator;
    static constexpr bool _MULTI = Traits::_MULTI;
  ...
  private:
    ...
    // Define inline because clang has trouble deducing the template arguments otherwise.
    template <class K, class... Args>
        requires detail::TryEmplaceConstraint<skip_list, K, Args...>
    iterator _try_emplace(K &&key, size_type level, Args &&...args) {
        auto predecessors = _find_predecessors(key);
  ...
  public:
    ...
    size_type erase(K &&key) {
        ...
        while (!_key_comp(key, lower._ptr->_key())) {
            auto next_it = std::next(lower);
            _deallocate_node(_extract_node(lower, predecessors));
            lower = next_it;
            ++count;
...
template <class Traits> struct skip_list<Traits, true>::_skip_list_node {
    friend skip_list;
    friend _iterator;
    friend _const_iterator;
...
template <class Traits> class skip_list<Traits, true>::_iterator {
    friend skip_list;
    friend _const_iterator;
...
template <class Traits> struct skip_list<Traits, true>::node_type {
    friend skip_list;
...
template <class Traits> class skip_list<Traits, true>::node_forward_guard {
  private:
    node_ptr _ptr;
    node_allocator_type _node_alloc;
    size_type _level;
...
template <class Traits> class skip_list<Traits, true>::copy_guard {
  private:
    std::unordered_map<node_ptr, node_ptr> _node_map; // later, use custom hash function
    vector<node_ptr> _owned_nodes;
    node_allocator_type _node_alloc;
...
namespace j {
template <class Traits> skip_list<Traits, true>::size_type skip_list<Traits, true>::_random_level() const {
    size_type level = 0;
    while (coin_flip(rng) && level <= MAX_LEVEL) {
        ++level;
    }
...
template <class Traits> auto skip_list<Traits, true>::_construct_node(size_type level) -> node_forward_guard {
    node_forward_guard node_guard(std::allocator_traits<node_allocator_type>::allocate(_node_alloc, 1), _node_alloc,
                                  level);
...
template <class Traits> void skip_list<Traits, true>::_deallocate_node(node_ptr node) noexcept {
    std::allocator_traits<node_allocator_type>::destroy(_node_alloc, &node->_value);
    using node_ptr_allocator = typename std::allocator_traits<node_allocator_type>::template rebind_alloc<node_ptr>;
    node_ptr_allocator _node_ptr_alloc(_node_alloc);
    std::allocator_traits<node_ptr_allocator>::deallocate(_node_ptr_alloc, node->_forward, node->_level + 1);
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
auto skip_list<Traits, true>::_find_predecessors(K &&key) const -> array<node_ptr, MAX_LEVEL + 1> {
    array<node_ptr, MAX_LEVEL + 1> predecessors;
    node_ptr current = _head;
    for (size_type i = _max_level + 1; i > 0; --i) { // avoid unsigned int underflow
        while (!_key_comp(key, current->_forward[i - 1]->_key())) {
            current = current->_forward[i - 1];
        }
        predecessors[i - 1] = current;
    }
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
auto skip_list<Traits, true>::_find_predecessors_lower(K &&key) const -> array<node_ptr, MAX_LEVEL + 1> {
    array<node_ptr, MAX_LEVEL + 1> predecessors;
    node_ptr current = _head;
    for (size_type i = _max_level + 1; i > 0; --i) { // avoid unsigned int underflow
        while (_key_comp(current->_forward[i - 1]->_key(), key)) {
            current = current->_forward[i - 1];
        }
        predecessors[i - 1] = current;
    }
...
template <class Traits>
auto skip_list<Traits, true>::_find_predecessors(const_iterator position) const -> array<node_ptr, MAX_LEVEL + 1> {
    array<node_ptr, MAX_LEVEL + 1> predecessors = _find_predecessors_lower(position._ptr->_key());
    node_ptr current_node = predecessors[0]->_forward[0];
...
// If we already know the predecessors and want to insert a new node after them,
template <class Traits>
void skip_list<Traits, true>::_update_predecessors(key_type key, array<node_ptr, MAX_LEVEL + 1> &predecessors) {
    for (size_type i = _max_level + 1; i > 0; --i) {
        while (!_key_comp(key, predecessors[i - 1]->_forward[i - 1]->_key())) {
            predecessors[i - 1] = predecessors[i - 1]->_forward[i - 1];
        }
    }
}
...
// Incrementally updates the 'predecessors' array from prev(position) to position.
template <class Traits>
void skip_list<Traits, true>::_advance_predecessors(array<node_ptr, MAX_LEVEL + 1> &predecessors) {
    node_ptr current_node = predecessors[0]->_forward[0];
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
bool skip_list<Traits, true>::_is_duplicate(K &&key, node_ptr next) const {
    return !_key_comp(key, next->_key()) && !_key_comp(next->_key(), key);
}

template <class Traits>
template <class V>
    requires std::constructible_from<typename Traits::value_type, V>
auto skip_list<Traits, true>::_init_node(V &&value, size_type level) -> node_forward_guard {
    node_forward_guard node_guard(std::move(_construct_node(level)));
    allocator_type _alloc = get_allocator();
    std::allocator_traits<allocator_type>::construct(_alloc, &node_guard.get()->_value, std::forward<V>(value));
...
template <class Traits> void skip_list<Traits, true>::_init_sentinels() {
    key_type min_key = detail::_key_limits_helper<key_type, key_compare>::min();
    key_type max_key = detail::_key_limits_helper<key_type, key_compare>::max();

    value_type min_value;
    if constexpr (_IS_SET) {
        min_value = min_key;
    } else {
        min_value = value_type{min_key, mapped_type()};
    }
    ...
    if constexpr (_IS_SET) {
        ...
        max_value = value_type{max_key, mapped_type()};
    }

    node_forward_guard head_guard(std::move(_init_node(std::move(min_value), MAX_LEVEL)));
    node_forward_guard tail_guard(std::move(_init_node(std::move(max_value), MAX_LEVEL)));
    for (size_type i = 0; i <= MAX_LEVEL; ++i) {
        head_guard.get()->_forward[i] = tail_guard.get();
        tail_guard.get()->_forward[i] = nullptr; // tail's forward is nullptr
    }
    head_guard.get()->_backward = nullptr; // head's backward is nullptr, tail's forward is nullptr
    tail_guard.get()->_backward = head_guard.get();
    _head = head_guard.release();
    _tail = tail_guard.release();
}

template <class Traits>
void skip_list<Traits, true>::_move_state(
    skip_list &&x) { // pre-require: _dummy is initialized (call _init_sentinels())
    using std::swap;
    swap(_head, x._head);
    swap(_tail, x._tail);
    swap(_max_level, x._max_level);
...
template <class Traits> template <class Strategy> void skip_list<Traits, true>::_clone_tree(const skip_list &other) {
    if (other.empty()) {
        _init_sentinels();
        _max_level = 0;
        _size = 0;
    ...
    _init_sentinels();
    copy_guard guard(_node_alloc);
    guard.reserve(other._size - 2); // except sentinels
    guard.insert_without_owning(other._head, _head);
    guard.insert_without_owning(other._tail, _tail);

    node_ptr current_other = other._head->_forward[0];
    while (current_other != other._tail) {
        if constexpr (Strategy::copy) {
            node_forward_guard new_guard(std::move(_init_node(current_other->_value, current_other->_level)));
            guard.insert(current_other, new_guard.get());
            new_guard.release();
    ...
    current_other = other._head->_forward[0];
    while (current_other != other._tail) {
        node_ptr new_node = guard.get_new_node(current_other);
    ...
    // set head and tail's forward and backward
    for (size_type i = 0; i <= other._max_level; ++i) {
        _head->_forward[i] = guard.get_new_node(other._head->_forward[i]);
    }
    _tail->_backward = guard.get_new_node(other._tail->_backward);
...
template <class Traits> void skip_list<Traits, true>::_destroy_tree() noexcept {
    for (node_ptr node = _head; node != _tail;) {
        node_ptr next = node->_forward[0];
        _deallocate_node(node);
        node = next;
    }
    _deallocate_node(_tail);
}

template <class Traits>
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::const_iterator skip_list<Traits, true>::_find_lower_bound(K &&key) const {
    node_ptr current = _head;
    for (size_type i = _max_level + 1; i > 0; --i) {
        while (_key_comp(current->_forward[i - 1]->_key(), key)) {
            current = current->_forward[i - 1];
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::const_iterator skip_list<Traits, true>::_find_upper_bound(K &&key) const {
    node_ptr current = _head;
    for (size_type i = _max_level + 1; i > 0; --i) {
        while (!_key_comp(key, current->_forward[i - 1]->_key())) {
            current = current->_forward[i - 1];
...
template <class Traits>
skip_list<Traits, true>::node_ptr
skip_list<Traits, true>::_extract_node(const_iterator position,
                                       const array<node_ptr, MAX_LEVEL + 1> &predecessors) noexcept {
    const size_type pos_level = position._ptr->_level;
    for (size_type i = 0; i <= pos_level; ++i) {
        predecessors[i]->_forward[i] = position._ptr->_forward[i];
...
template <class Traits>
void skip_list<Traits, true>::_insert_node(node_ptr new_node, array<node_ptr, MAX_LEVEL + 1> &predecessors) noexcept {
    if (new_node->_level > _max_level) {
        std::fill(predecessors.begin() + _max_level + 1, predecessors.begin() + new_node->_level + 1, _head);
        _max_level = new_node->_level;
    }
...
template <class... Args>
    requires std::constructible_from<typename Traits::value_type, Args &&...>
std::pair<typename skip_list<Traits, true>::iterator, bool> skip_list<Traits, true>::_emplace(size_type level,
                                                                                              Args &&...args) {
    value_type val(std::forward<Args>(args)...);
    key_type key;
    if constexpr (_IS_SET) {
...
template <class Traits>
skip_list<Traits, true>::skip_list(const key_compare &comp, const allocator_type &alloc)
    : _node_alloc(alloc), _key_comp(comp) {
    rng.seed(std::random_device{}());
    _init_sentinels();
    _max_level = 0;
...
template <class Traits>
skip_list<Traits, true>::skip_list(const skip_list &other)
    : _node_alloc(std::allocator_traits<allocator_type>::select_on_container_copy_construction(other._node_alloc)),
      _key_comp(other._key_comp) {
    _clone_tree<_strategy_copy>(other);
}
...
template <class Traits>
skip_list<Traits, true>::skip_list(const skip_list &other, const std::type_identity_t<allocator_type> &alloc)
    : _node_alloc(alloc), _key_comp(other._key_comp) {
    _clone_tree<_strategy_copy>(other);
}

template <class Traits>
skip_list<Traits, true>::skip_list(skip_list &&x)
    : rng(std::move(x.rng)), coin_flip(std::move(x.coin_flip)), _max_level(0), _node_alloc(std::move(x._node_alloc)),
      _size(0), _key_comp(std::move(x._key_comp)) {
    _init_sentinels();
    _move_state(std::move(x));
...
template <class Traits>
skip_list<Traits, true>::skip_list(skip_list &&x, const std::type_identity_t<allocator_type> &alloc)
    : rng(std::move(x.rng)), coin_flip(std::move(x.coin_flip)), _max_level(0), _node_alloc(std::move(x._node_alloc)),
      _size(0), _key_comp(std::move(x._key_comp)) {
    if constexpr (!std::allocator_traits<allocator_type>::is_always_equal::value) {
        if (_node_alloc != x._node_alloc) {
...
template <class Traits> skip_list<Traits, true> &skip_list<Traits, true>::operator=(const skip_list &x) {
    if (this == std::addressof(x)) {
        return *this;
    }
...
template <class Traits>
skip_list<Traits, true> &skip_list<Traits, true>::operator=(skip_list &&x) noexcept(
    std::allocator_traits<allocator_type>::is_always_equal::value && std::is_nothrow_move_assignable_v<key_compare>) {
    if (this == std::addressof(x)) {
        return *this;
    }
...
template <class Traits> skip_list<Traits, true>::~skip_list() noexcept {
    _destroy_tree();
}

template <class Traits> skip_list<Traits, true>::iterator skip_list<Traits, true>::begin() noexcept {
    return iterator(_head->_forward[0]);
}

template <class Traits> skip_list<Traits, true>::const_iterator skip_list<Traits, true>::cbegin() const noexcept {
    return const_iterator(_head->_forward[0]);
}

template <class Traits> skip_list<Traits, true>::iterator skip_list<Traits, true>::end() noexcept {
    return iterator(_tail);
}

template <class Traits> skip_list<Traits, true>::const_iterator skip_list<Traits, true>::cend() const noexcept {
    return const_iterator(_tail);
}

template <class Traits> bool skip_list<Traits, true>::empty() const noexcept {
    return _size == 0;
}

template <class Traits> skip_list<Traits, true>::size_type skip_list<Traits, true>::size() const noexcept {
    return _size;
}

template <class Traits> skip_list<Traits, true>::size_type skip_list<Traits, true>::max_size() const noexcept {
    return std::allocator_traits<allocator_type>::max_size(get_allocator());
}

template <class Traits>
skip_list<Traits, true>::allocator_type skip_list<Traits, true>::get_allocator() const noexcept {
    return allocator_type(_node_alloc);
}
...
template <class... Args>
    requires std::constructible_from<typename Traits::value_type, Args &&...>
std::pair<typename skip_list<Traits, true>::iterator, bool> skip_list<Traits, true>::emplace(Args &&...args) {
    size_type new_node_level = _random_level();
    return _emplace(new_node_level, std::forward<Args>(args)...);
}
...
template <class... Args>
    requires std::constructible_from<typename Traits::value_type, Args &&...>
skip_list<Traits, true>::iterator skip_list<Traits, true>::emplace_hint(const_iterator position, Args &&...args) {
    value_type val(std::forward<Args>(args)...);
    key_type key;
    if constexpr (_IS_SET) {
        key = val;
...
template <class InputIter>
    requires(std::input_iterator<InputIter> &&
             std::constructible_from<typename Traits::value_type, std::iter_reference_t<InputIter>>)
void skip_list<Traits, true>::insert(InputIter first, InputIter last) {
    if (first == last)
        return;

    array<node_ptr, MAX_LEVEL + 1> predecessors;
    predecessors.fill(_head);
    ...
    for (; first != last; ++first) {
        ...
        if (!_key_comp(key, predecessors[0]->_key())) {
            _update_predecessors(key, predecessors);
        } else {
            predecessors = _find_predecessors(key);
            std::fill(predecessors.begin() + _max_level + 1, predecessors.end(), _head);
        }
        if constexpr (!_MULTI) {
            if (_is_duplicate(key, predecessors[0])) {
                continue;
...
template <class Traits> skip_list<Traits, true>::node_type skip_list<Traits, true>::extract(const_iterator position) {
    return node_type{_extract_node(position, _find_predecessors(position)), get_allocator()};
}

template <class Traits> skip_list<Traits, true>::insert_return_type skip_list<Traits, true>::insert(node_type &&nh) {
    auto predecessors = _find_predecessors(nh._ptr->_key());
    if constexpr (!_MULTI) {
        if (_is_duplicate(nh._ptr->_key(), predecessors[0])) {
            return {iterator(predecessors[0]), false, std::move(nh)};
...
template <class Traits>
skip_list<Traits, true>::iterator skip_list<Traits, true>::insert(const_iterator position, node_type &&nh) {
    size_type new_node_level = nh._ptr->_level;
    key_type key = nh._ptr->_key();
...
template <class Traits> skip_list<Traits, true>::iterator skip_list<Traits, true>::erase(const_iterator position) {
    auto next_it = iterator(position._ptr->_forward[0]);
    _deallocate_node(_extract_node(position, _find_predecessors(position)));
    return next_it;
}
...
template <class Traits>
skip_list<Traits, true>::iterator skip_list<Traits, true>::erase(const_iterator first, const_iterator last) {
    array<node_ptr, MAX_LEVEL + 1> predecessors = _find_predecessors(first);
...
template <class Traits>
void skip_list<Traits, true>::swap(skip_list &x) noexcept(
    std::allocator_traits<allocator_type>::is_always_equal::value && std::is_nothrow_swappable_v<key_compare>) {
    using std::swap;
    if constexpr (std::allocator_traits<typename Traits::allocator_type>::propagate_on_container_swap::value) {
        swap(_node_alloc, x._node_alloc);
...
template <class Traits> void skip_list<Traits, true>::clear() noexcept {
    for (node_ptr node = _head->_forward[0]; node != _tail;) {
        node_ptr next = node->_forward[0];
        _deallocate_node(node);
        node = next;
    }
    for (size_type i = 0; i <= _max_level; ++i) {
        _head->_forward[i] = _tail;
    }
    _tail->_backward = _head;
    _max_level = 0;
    _size = 0;
}
...
// template <class SourceTree>
// requires IsMergeable<Traits, SourceTree>
// void skip_list<Traits, true>::merge(SourceTree &source) {}

template <class Traits> void skip_list<Traits, true>::merge(skip_list &source) {
    if (this == std::addressof(source) || source.empty()) {
        return;
    }
    array<node_ptr, MAX_LEVEL + 1> predecessors, source_predecessors;
    predecessors.fill(_head);
    source_predecessors.fill(source._head);
...
template <class Traits> skip_list<Traits, true>::key_compare skip_list<Traits, true>::key_comp() const {
    return _key_comp;
}

template <class Traits> skip_list<Traits, true>::value_compare skip_list<Traits, true>::value_comp() const {
    return value_compare(_key_comp);
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::iterator skip_list<Traits, true>::find(K &&key) {
    const_iterator cit = std::as_const(*this).find(std::forward<K>(key));
    return iterator(cit._ptr);
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::const_iterator skip_list<Traits, true>::find(K &&key) const {
    const_iterator it = _find_lower_bound(std::forward<K>(key));
    value_compare value_comp = this->value_comp();
    if (it != cend() && !value_comp(key, *it) && !value_comp(*it, key)) {
        return it;
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::size_type skip_list<Traits, true>::count(K &&key) const {
    if constexpr (_MULTI) {
        auto range = equal_range(std::forward<K>(key));
        return std::distance(range.first, range.second);
    } else {
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
bool skip_list<Traits, true>::contains(K &&key) const {
    return find(std::forward<K>(key)) != cend();
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::iterator skip_list<Traits, true>::lower_bound(K &&key) {
    const_iterator cit = _find_lower_bound(std::forward<K>(key));
    return iterator(cit._ptr);
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::const_iterator skip_list<Traits, true>::lower_bound(K &&key) const {
    return _find_lower_bound(std::forward<K>(key));
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::iterator skip_list<Traits, true>::upper_bound(K &&key) {
    const_iterator cit = _find_upper_bound(std::forward<K>(key));
    return iterator(cit._ptr);
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
skip_list<Traits, true>::const_iterator skip_list<Traits, true>::upper_bound(K &&key) const {
    return _find_upper_bound(std::forward<K>(key));
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
std::pair<typename skip_list<Traits, true>::iterator, typename skip_list<Traits, true>::iterator>
skip_list<Traits, true>::equal_range(K &&key) {
    auto lower = _find_lower_bound(std::forward<K>(key));
    auto upper = _find_upper_bound(std::forward<K>(key));
    return {iterator(lower._ptr), iterator(upper._ptr)};
}
...
template <class K>
    requires IsTransparentlyComparable<K, typename Traits::key_type, typename Traits::key_compare>
std::pair<typename skip_list<Traits, true>::const_iterator, typename skip_list<Traits, true>::const_iterator>
skip_list<Traits, true>::equal_range(K &&key) const {
    return {_find_lower_bound(std::forward<K>(key)), _find_upper_bound(std::forward<K>(key))};
}
