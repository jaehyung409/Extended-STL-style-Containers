...
Iter uninitialized_fill_n_contiguous(Allocator &alloc, Iter first, Size count, const T &value) {
    ...
    if constexpr (std::is_same_v<ValueType, T> && std::is_trivially_constructible_v<ValueType> &&
                  std::is_trivially_copyable_v<ValueType> && std::is_standard_layout_v<ValueType>) {
        if (value == ValueType{}) {
            std::memset(std::to_address(first), 0, count * sizeof(ValueType));
            return first + count;
        }
